...
/** Port of React Bits' WarpText: renders the headline into a canvas
 *  texture, then distorts it in a WebGL fragment shader (ambient fbm
 *  noise + a cursor-lensing bulge with a soft RGB-split "glass" edge). */
...
const drawLine = (ctx: CanvasRenderingContext2D, line: string, anchorX: number, y: number, letterSpacing: number, align: 'left' | 'center') => {
  const chars = Array.from(line)
  let cursor = align === 'center' ? anchorX - measureLine(ctx, line, letterSpacing) / 2 : anchorX
...
const buildTextCanvas = ({ container, width, height, dpr, props }: { container: HTMLElement; width: number; height: number; dpr: number; props: RasterProps }) => {
  ...
  const maxWidth = width * (props.textAlign === 'left' ? 0.98 : 0.86)
  ...
  const anchorX = props.textAlign === 'left' ? 0 : width / 2
  lines.forEach((line, index) => drawLine(ctx, line, anchorX, startY + index * lineHeight, letterSpacing, props.textAlign))

  return canvas
}

interface WarpUniformProps {
  warpStrength: number
  warpScale: number
  speed: number
  pointerInfluence: number
...
const syncUniforms = (program: Program, props: WarpUniformProps) => {
...
export function WarpText({
  ...
  const propsRef = useRef({ text, color, fontSize, fontWeight, fontFamily, letterSpacing, lineHeight, textAlign, warpStrength, warpScale, speed, pointerInfluence, pointerStrength, refraction, ripple })
  const contextRef = useRef<{ program: Program; rasterize: () => void } | null>(null)

  useEffect(() => {
    propsRef.current = { text, color, fontSize, fontWeight, fontFamily, letterSpacing, lineHeight, textAlign, warpStrength, warpScale, speed, pointerInfluence, pointerStrength, refraction, ripple }

    if (contextRef.current) {
      syncUniforms(contextRef.current.program, propsRef.current)
      contextRef.current.rasterize()
    }
  }, [text, color, fontSize, fontWeight, fontFamily, letterSpacing, lineHeight, textAlign, warpStrength, warpScale, speed, pointerInfluence, pointerStrength, refraction, ripple])
  ...
  useEffect(() => {
    ...
    const rasterize = async () => {
      ...
      const textCanvas = buildTextCanvas({ container, width: rect.width, height: rect.height, dpr, props: propsRef.current })
